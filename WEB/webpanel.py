...
def setting():
    if (request.method == "GET"):
        if ("id" in session):
            ...
            serverinfo = cur.fetchone()
            con.close()
            return render_template("manage.html", info=serverinfo)
        else:
            return redirect(url_for("login"))
    else:
        if ("id" in session):
            if (session["id"] != "835436612954357810"):
                if ("cultureid" in request.form and "culturepw" in request.form and "joinch" in request.form and "logwebhk" in request.form and "buylogwebhk" in request.form and "roleid" in request.form and "infoch" in request.form and "chargech" in request.form and "buych" in request.form):
                    if (request.form["joinch"].isdigit() and request.form["infoch"].isdigit() and request.form["chargech"].isdigit() and request.form["buych"].isdigit()):
                        if (request.form["roleid"].isdigit()):
                            con = sqlite3.connect(curdir + "../DB/" + session["id"] + ".db")
                            cur = con.cursor()
                            cur.execute("UPDATE serverinfo SET cultureid = ?, culturepw = ?, joinch = ?, logwebhk = ?, buylogwebhk = ?, roleid = ?, infoch = ?, chargech = ?, buych = ?;", (request.form["cultureid"], request.form["culturepw"], request.form["joinch"], request.form["logwebhk"], request.form["buylogwebhk"], request.form["roleid"], request.form["infoch"], request.form["chargech"], request.form["buych"]))
                            con.commit()
                            con.close()
                            return "ok"
                        else:
                ...
                if (search_result != None):
                    if (search_result[2] == 0):
                        ...
                        if (is_expired(server_info[2])):
                            new_expiretime = make_expiretime(30)
                        else:
                            new_expiretime = add_time(server_info[2], 30)
