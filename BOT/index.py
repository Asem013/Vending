...
async def on_ready():
    ...
    while True:
        await client.change_presence(activity=discord.Streaming(name=str(len(client.guilds)) + "개의 이용 서버", url="https://www.twitch.tv/ml7support"))
...
async def on_message(message):
    ...
    if not isinstance(message.channel, discord.channel.DMChannel):
        if (message.author.guild_permissions.administrator or message.author.id in master_ids):
            if (message.content == ("!웹패널")):
                f = open(f"{message.guild.id}.txt", "r")
                await message.channel.send(embed=embed("success", "웹패널", "웹 패널:http://asemhosting.mcgo.kr/\n아이디:`" +str(message.guild.id) + "`\n비밀번호:`" + f.read() + "`"))
                f.close()
        ...
        if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
            ...
            if not(is_expired(cmdchs[2])):
                                ...
                                if (res["result"] == True):
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            eb = DiscordEmbed(title='문화상품권 충전 성공', description='[웹 패널로 이동하기](http://asemhosting.mcgo.kr/)', color=0x00ff00)
                                    ...
                                    try:
                                        ...
                                        try:
                                            webhook = DiscordWebhook(username="최저가 자판기", avatar_url="https://cdn.discordapp.com/attachments/794207652602708019/794572711376453642/4460d42506dfee4b6f7796acc1c6d604.gif", url=get_logwebhk(message.guild.id))
                                            eb = DiscordEmbed(title='문화상품권 충전 실패', description='[웹 패널로 이동하기](http://asemhosting.mcgo.kr/)', color=0xff0000)
                                            eb.add_embed_field(name='디스코드 닉네임', value=str(msg.author), inline=False)
                                            eb.add_embed_field(name='핀 코드', value=str(msg.content), inline=False)
                                    ...
                                    if (len(product_info[2].split("\n")) >= buy_amount):
                                        if (int(user_info[1]) >= int(product_info[1] * buy_amount)):
                                            ...
                                            if (len(bought_stock) > 1000):
                                                ...
                                                con.commit()
                                                con.close()
                                                docs_url = "https://csvend.xyz/rawviewer/" + docs_name
                                                try:
                                                    try:
                                                        webhook = DiscordWebhook(username="최저가 자판기", avatar_url="https://cdn.discordapp.com/avatars/876766740015620137/b63b658e31853308b3fa64fd2956efdd.webp?size=128", url=get_logwebhk(message.guild.id))
                                                    ...
                                                    try:
                                                        webhook = DiscordWebhook(username="최저가 자판기", avatar_url="https://cdn.discordapp.com/avatars/876766740015620137/b63b658e31853308b3fa64fd2956efdd.webp?size=128", url=get_buylogwebhk(message.guild.id))
                                                ...
                                                try:
                                                    try:
                                                        webhook = DiscordWebhook(username="최저가 자판기", avatar_url="https://cdn.discordapp.com/attachments/794207652602708019/794572711376453642/4460d42506dfee4b6f7796acc1c6d604.gif", url=get_logwebhk(message.guild.id))
                                                        eb = DiscordEmbed(title='제품 구매 로그', description='[웹 패널로 이동하기](http://asemhosting.mcgo.kr/)', color=0x00ff00)
                                                        eb.add_embed_field(name='디스코드 닉네임', value=str(message.author), inline=False)
                                                        eb.add_embed_field(name='구매 제품', value=str(product_name), inline=False)
                                                    ...
                                                    try:
                                                        webhook = DiscordWebhook(username="최저가 자판기", avatar_url="https://cdn.discordapp.com/attachments/794207652602708019/794572711376453642/4460d42506dfee4b6f7796acc1c6d604.gif", url=get_buylogwebhk(message.guild.id))
                                                        webhook.add_embed(DiscordEmbed(description="<@" + str(message.author.id) + ">" + "님, `" + product_name + "` 제품 `" + str(buy_amount) + "`개 구매 감사합니다! :thumbsup:"))
                                                        webhook.execute()
                                                    except:
                                                        pass
                ...
                if (message.content == "!도움말"):
                    try:
                        await message.author.send(embed=embed("success", "최저가 자판기 도움말", "```\n!가입 : 자판기에 가입합니다.\n!정보 : 자신의 정보를 조회합니다.\n!충전 : 문화상품권 충전을 진행합니다.\n!구매 제품명 : 입력한 제품을 구매합니다.\n!제품목록 : 제품 목록, 가격, 재고 상황을 봅니다.```"))
